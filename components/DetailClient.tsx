    ...
    <main className="min-h-screen bg-stone-50">
      <header className="border-b border-stone-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-5 sm:px-6 lg:px-8">
          <div>
            <Link className="mb-2 inline-flex items-center gap-2 text-sm font-medium text-stone-500 hover:text-rose-700" href="/dashboard">
              <ArrowLeft className="size-4" aria-hidden="true" />
              Back to dashboard
            </Link>
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-2xl font-semibold text-stone-950">{getProfileName(client)}</h1>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,1fr)_440px] lg:px-8">
        <section className="space-y-5">
          {groupedFields.map((group) => (
            <div key={group.title} className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center gap-2 text-stone-950">
                <span className="flex size-8 items-center justify-center rounded-md bg-rose-50 text-rose-700">
                  {group.icon}
                </span>
              ...
              <dl className="grid gap-3 sm:grid-cols-2">
                {group.fields.map(([label, value]) => (
                  <div key={String(label)} className="border-b border-stone-100 pb-2">
          ...
          <div className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="text-base font-semibold text-stone-950">Matchmaker Notes</h2>
            </div>
            <textarea
              value={notes}
              onChange={(event) => setNotes(event.target.value)}
              className="min-h-32 w-full resize-y rounded-md border border-stone-200 bg-stone-50 p-3 text-sm text-stone-900 outline-none focus:border-rose-400 focus:bg-white"
        ...
        <aside className="space-y-4">
          <div className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
            <div className="mb-1 flex items-center gap-2">
              ...
              <h2 className="text-base font-semibold text-stone-950">Suggested Matches</h2>
            </div>
          ...
          {matches.slice(0, 8).map((candidate) => (
            <article key={candidate.id} className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="font-semibold text-stone-950">{getProfileName(candidate)}</h3>
  ...
  return (
    <div className="mt-3 rounded-md border border-rose-100 bg-rose-50 p-3">
      <div className="mb-2 flex items-center justify-between gap-3">
        ...
        <span className="text-xs font-bold text-stone-950">{state.data.compatibilityScore}/100</span>
      </div>
      <p className="text-sm leading-6 text-stone-700">{state.data.reasoning}</p>
    </div>
  );
}
      ...
      <div className="w-full max-w-2xl rounded-lg bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-stone-200 px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-stone-950">Mock Outreach Email</h2>
          </div>
          <button onClick={onClose} className="rounded-md p-2 text-stone-500 hover:bg-stone-100" aria-label="Close modal">
            <X className="size-5" aria-hidden="true" />
          </button>
        </div>
        <div className="space-y-4 p-5 text-sm leading-6 text-stone-700">
          <p className="font-medium text-stone-950">Subject: A thoughtful introduction from The Date Crew</p>
          <p>Hi {client.firstName},</p>
          <p>{intro}</p>
          <div className="rounded-md border border-stone-200 bg-stone-50 p-3">
            <p className="font-semibold text-stone-950">{getProfileName(candidate)}</p>
            <p>
